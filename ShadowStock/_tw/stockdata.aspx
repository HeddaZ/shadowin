...
<script runat="server">
        ...
        foreach (string stockCode in stockCodes)
        {
            switch (stockCode.Substring(0, 2).ToLower())
            {
                case "tw": //台湾
                    ...
                    string beginStamp = "<td align=center width=105>";
                    string endStamp = "<td align=center width=137 class=\"tt\">";
                    ...
                    stockDataHtml = Regex.Replace(stockDataHtml, @"\s+|\,+", string.Empty);
                    stockDataHtml = Regex.Replace(stockDataHtml, @"\<[^<>]+\>", ",");
                    stockDataHtml = Regex.Replace(stockDataHtml, @"\,{2,}", ",").Trim(',');
    ...
    {
        return this.GetData(url, Encoding.Default);
    }
</script>
